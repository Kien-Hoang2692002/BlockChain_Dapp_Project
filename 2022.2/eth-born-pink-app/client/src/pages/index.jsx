...
const Home = () => {
  ...
  const registerManager = async () => {
    try {
      await contract.methods.addManager().send({ from: accounts[0] })
      dispatch({
        type: 'ADD_MANAGER',
      })
    } catch (err) {
      ...
      if (role === 'unknown') {
        return (
          <Box display='flex' flexDirection='column' alignItems='center'>
            <Box mb={2}>
              <CustomButton text='Manager Register' handleClick={() => registerManager()}>
                <PersonAddAlt1RoundedIcon style={{ color: 'white' }} />
              </CustomButton>
            </Box>
            <Typography variant='h5' color='white'>
              If you are a blink, ask your manager to register for you
            </Typography>
          </Box>
        )
      } else if (role === 'blink') {
        return (
          <CustomButton text='Blink Portal' handleClick={() => navigate('/blink')}>
            <LoginRoundedIcon style={{ color: 'white' }} />
          </CustomButton>
        )
      } else if (role === 'manager') {
        return (
          <CustomButton text='Manager Portal' handleClick={() => navigate('/manager')}>
            <LoginRoundedIcon style={{ color: 'white' }} />
          </CustomButton>
        )
      }
